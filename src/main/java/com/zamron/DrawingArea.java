...
public class DrawingArea extends QueueNode {
	...
	public static void drawTransparentBox(int leftX, int topY, int width, int height, int rgbColour, int opacity) {
		if (leftX < DrawingArea.topX) { // if leftX is less than topX
			...
			leftX = DrawingArea.topX; // set leftX equal to topX
		}
		if (topY < DrawingArea.topY) { // same thing as above but with Y/height
			height -= DrawingArea.topY - topY;
			topY = DrawingArea.topY;
		}
		if (leftX + width > bottomX) // if sum of leftX/Width is greater than bottomX
			width = bottomX - leftX; // set width equal to the sum of bottomX - leftX
		if (topY + height > bottomY) // same as above just with Y/height
			height = bottomY - topY;
		...
		int leftOver = DrawingArea.width - width; // set leftOver equal to the drawing area's width substracted by the
													// width in params
		int pixelIndex = leftX + topY * DrawingArea.width; // set pixel index equal to the sum of (leftX * topY *
															// DrawingArea.width)
		for (int rowIndex = 0; rowIndex < height; rowIndex++) { // loop thru all the y pixels (i guess)
			for (int columnIndex = 0; columnIndex < width; columnIndex++) { // loop thru all the x pixels (i guess)
				int otherRed = (pixels[pixelIndex] >> 16 & 0xff) * transparency; // set otherRed equal to pixels at
																					// pixelIndex shifted by 16 bits to
																					// the left & 0xff
				int otherGreen = (pixels[pixelIndex] >> 8 & 0xff) * transparency; // same as above just 8 bits shifted
																					// to the left
				...
				int transparentColour = ((red + otherRed >> 8) << 16) + ((green + otherGreen >> 8) << 8) // calculating
																											...
																											// transparentColour
						+ (blue + otherBlue >> 8);
				pixels[pixelIndex++] = transparentColour; // set pixels[pixelIndex] incremented every iteration equal to
															// transparentColour
			}
	...
	private static void drawVerticalStrokeLine(int xPosition, int yPosition, int height, int hexColor,
			int strokeWidth) { // wrote this bitch ass method myself
