...
public class Client extends RSApplet {
	...
	public static YellowAnnouncement yellowAnnouncement;
	...
	RSInterface itemInterface1 = null;
	RSInterface itemInterface2 = null;

	public boolean yellowEnded = true;
	public boolean yellowStarted = false;
	public boolean yellowHasFinished = true;

	public boolean osrsStarted = false;
	public boolean osrsHasFinished = true;
	public boolean isDisplayed = true;
	public static OSRSBroadCast broadcast;
	private boolean canDraw = false;

	CustomAlertInterface customAlert = new CustomAlertInterface(450, 135, 200, 0x9695cf, 150, 100, 350);

	public boolean npcEditing = false;

	String desc = "";

	public void sendNpcConfigPacket(int npcId, int direction, int xPos, int yPos, int zPos, boolean coordinate,
			int radius) {
	...
	public void sendTransformationId(int npcId) {
	...
	public List<String> builtTextList = new ArrayList<>();
	...
	public void askData() {
		...
		int itemId = currentlyEditedItemId;

		stream.createFrame(201);

		stream.writeWord(itemId);

		//System.out.println("Asked data about item " + itemId + " from the server");

	}
	public void sendColors(int id, int[] initialColors, int[] targetColors) {
		//System.out.println("Data to write: " + id + " | " + Arrays.toString(initialColors) + " | "
				//+ Arrays.toString(targetColors));
		stream.createFrame(213);
		stream.writeWordBigEndian(2 + (initialColors.length * 4) + (targetColors.length * 4));
		stream.writeWord(id);
		for (int ic : initialColors) {
			stream.writeDWord(ic);
		}

		for (int tc : targetColors) {
			stream.writeDWord(tc);
		}
	}
	...
	public void preloadModels() {
		...
		//System.out.println("Total preloaded model count: " + i);
	}
	 ...
	 private void handleSettings() throws IOException {
	  ...
	  public void loadSettings() {
	        if (!FileOperations.FileExists(signlink.findcachedir()
	            ...
	            options.put("hide_pet_options", Boolean.valueOf(false));
	            options.put("hide_pets", Boolean.valueOf(false));
	            options.put("loyalty_box", Boolean.valueOf(true));
	            shadowIndex = 4;
	        ...
	        try {
	            ...
	            options.put("hide_pet_options", Boolean.valueOf(in.readByte() == 1));
	            options.put("hide_pets", Boolean.valueOf(in.readByte() == 1));
	            options.put("loyalty_box", Boolean.valueOf(in.readByte() == 1));
	    ...
	    public void saveSettings() {
	        try {
	            ...
	            out.write(getOption("hide_pet_options") ? 1 : 0);
	            out.write(getOption("hide_pets") ? 1 : 0);
	            out.write(getOption("loyalty_box") ? 1 : 0);
	...
	public String getPrefix(int rights, int secondaryrights, int ironman) {
		...
		String primaryRank = rights > 0 ? "<img=" + rights + ">" : "";
		String secondaryRank = secondaryrights > 0 ? "<zmg=" + secondaryrights + ">" : "";
		String finalPrefix = ironman > 0 ? ironRank + secondaryRank : primaryRank + secondaryRank;

		//System.out.println("FINAL PREFIX 2144241 WAS " + finalPrefix);
		return finalPrefix;
	}

	public int getPrimaryRights(String prefix) {
	...
	public int getSecondaryRights(String prefix) {
		...
		if (prefix.contains("<zmg=2>")) {
			secondaryRights1 = 2;
		}
		if (prefix.contains("<zmg=3>")) {
			secondaryRights1 = 3;
		}
		if (prefix.contains("<zmg=4>")) {
			secondaryRights1 = 4;
		}
		if (prefix.contains("<zmg=5>")) {
			secondaryRights1 = 5;
		}
	...
	public static int clientWidth = 774, clientWidth2 = 774, clientHeight = 512, clientHeight2 = 512;																// original
	public static int chatAreaHeight = 0;
	private int gameAreaWidth = 512, gameAreaHeight = 334;
	public int appletWidth = 774, appletHeight = 512;
	...
	public void drawChatArea() {
		...
		if (showChat) {
			if (clientSize == 0) {
				...
				cacheSprite[0].drawARGBSprite(0 + offsetX, 0 + offsetY);
			}
		}
		...
		if (showInput) {
			...
			for (int index = 0; index < 500; index++) {
				...
				if (chatMessages[index] != null) {
					...
					if (chatType == 0) {
						//System.out.println("Chat type was 0!!!!!");
					...
					if ((chatType == 1 || chatType == 2)
						...
						//System.out.println("CHAT TYPE "+chatType + " Public Chat Mode = "+ publicChatMode);
						if (chatTypeView == 1 || chatTypeView == 0 || (playerRights > 0)) {
							if (positionY > 0 && positionY < 210) {
								int xPos = 11;
								...
								if (playerRights > 0 && name.contains("<zmg")) {
									newRegularFont.drawBasicString(name + ":", xPos + offsetX + 3, positionY + offsetY, 0, -1);
								} else {
									newRegularFont.drawBasicString(name + ":", xPos + offsetX, positionY + offsetY, 0, -1);
								}
					...
					if ((chatType == 3 || chatType == 7) && (splitPrivateChat == 0 || chatTypeView == 2)
						...
						if (chatTypeView == 2 || chatTypeView == 0) {
							if (positionY > 0 && positionY < 210) {
								...
								newRegularFont.drawBasicString("From", xPos + offsetX, positionY + offsetY, 0, -1);
								xPos += newRegularFont.getTextWidth("From ");
								if ((playerRights > 0 || ironman2 > 0) && ironman2 != 3) {
									if (ironman2 > 0 && playerRights == 0) {
										...
										SpriteLoader.sprites[1249 + playerRights].drawSprite(xPos + 1 + offsetX,
												positionY - 11 + offsetY);
										xPos += SpriteLoader.sprites[1249+ playerRights].myWidth+4;

									}

									if (secondaryRights > 0) {
										//System.out.println("Secondary rights: " + secondaryPlayerRights);
										SpriteLoader.sprites[1253+secondaryRights].drawSprite(xPos + 1 + offsetX,
												positionY - 11 + offsetY);
										xPos += SpriteLoader.sprites[1253+secondaryRights].myWidth+6;
									}
								}
								if(ironman2 == 3) {
					...
					if (chatType == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(name))) { // TODO -Lel-
						if (chatTypeView == 3 || chatTypeView == 0) {
							if (positionY > 0 && positionY < 210) {
								newRegularFont.drawBasicString(name + " " + chatMessages[index], 11 + offsetX,
										positionY + offsetY, 0x800080, -1);
							}
							scrollPosition++;
							messageY++;
						}
					...
					if (chatType == 16) {
						if (chatTypeView == 11 || chatTypeView == 0) {
							if (positionY > 0 && positionY < 210) {
								...
								newRegularFont.drawBasicString("" + message, positionX, positionY + offsetY, 0, -1);
							}
							scrollPosition++;
							messageY++;
						}
					...
					if (chatType == 21 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(name))) {
						if (chatTypeView == 3 || chatTypeView == 0) {
							if (positionY > 0 && positionY < 210) {
								newRegularFont.drawBasicString(name + " " + chatMessages[index], 11 + offsetX,
										positionY + offsetY, 0xf01322, -1);
							}
							scrollPosition++;
							messageY++;
						}
			...
			textDrawingArea.drawRegularText(false, 11 + offsetX, 0, name, 133 + offsetY);
			cacheSprite[14].drawSprite(textDrawingArea.getTextWidth(name) + 11 + offsetX, 123 + offsetY);
			textDrawingArea.drawRegularText(false, textDrawingArea.getTextWidth(name) + 24 + offsetX, 0, ": ",
					133 + offsetY);
	 ...
	 * @param dumpByName
	 */
	public void dumpItemImages(boolean dumpByName) { // int id, int stackSize, int backColour
	...
	public void dumpImage(Sprite image, String name) {
		...
		try {
			File out = new File(signlink.findcachedir() + "rsimg/dump/" + name + ".png");
	...
	public void say(String text, boolean isQuickSay) {
		...
		if (myRights > 0)
			gaps = " ";*/
		pushMessage("1111111111111111"+myPlayer.textSpoken, 2,
				getPrefix(myRights, secondaryRights, ironman) + (glowColor != 0 ? "@glo" + glowColor + "@" : "")
						+  myPlayer.name,
	...
	public void drawChannelButtons(int xPosOffset, int yPosOffset) {
		...
		smallText.drawCenteredText(0xffffff, CHANNELBUTTON_X[2] + 28 + xPosOffset, "@cya@Public", 152 + yPosOffset,
				true);
	...
	private void rightClickChatButtons() {
		...
		if (super.mouseX >= x[0] && super.mouseX <= x[0] + 56 && super.mouseY >= clientHeight - 23
			 ...
			 * else if (super.mouseX >= x[5] && super.mouseX <= x[5] + 56 && super.mouseY >=
			 * clientHeight - 23 && super.mouseY <= clientHeight) { menuActionName[1] =
			 * "Off Trade"; menuActionID[1] = 987; menuActionName[2] = "Friends Trade";
			 * menuActionID[2] = 986; menuActionName[3] = "On Trade"; menuActionID[3] = 985;
			 * menuActionName[4] = "View Trade"; menuActionID[4] = 984; menuActionRow = 5; }
			 * else if (super.mouseX >= x[6] && super.mouseX <= x[6] + 56 && super.mouseY >=
			 * clientHeight - 23 && super.mouseY <= clientHeight) { menuActionName[1] =
			 * "Hide Yell"; menuActionID[1] = 983; menuActionName[2] = "Off Yell";
			 * menuActionID[2] = 982; menuActionName[3] = "Friends Yell"; menuActionID[3] =
			 * 981; menuActionName[4] = "On Yell"; menuActionID[4] = 980; menuActionName[5]
			 * = "View Yell"; menuActionID[5] = 979; menuActionRow = 5; }
			 */
	}
	...
	public void rightClickMapArea() {
		...
		if (mouseInRegion(522, 121, 559, 157)) {
			menuActionName[menuActionRow] = "@red@Teleports";
	...
	private void updateItemData(ItemDef def) {
		RSInterface.interfaceCache[57372].message = "@gre@" + def.name;
		RSInterface.interfaceCache[57373].message = desc.length() > 1 ? desc : "Description not found";
	}
	...
	private void drawTabArea() {
		...
		if (clientSize == 0) {
			int xPos = 516;
			int yPos = 169;
			if (super.mouseX > xPos && super.mouseY > yPos && itemId > -1 && RSApplet.controlling) {
				ItemDef def = ItemDef.forID(itemId);
				for (Object[] i : ItemDef.customDescData) {
					int id = (int) i[0];
					desc = (String) i[1];
					if (def.id == id) {
						//System.out.println("id is: " + id);
						//System.out.println("Desc is: " + desc);
						drawInterface(0, getXPos(true, xPos, false), RSInterface.interfaceCache[57370],
								getYPos(true, yPos, false));
						updateItemData(def);
						//System.out.println("Custom desc: " + def.customDescription + " for id: " + def.id);
					}
				}
			}
		}
		// int scrollOffset, int interfaceX, RSInterface rsInterface, int interfaceY
		if (clientSize == 0) {
	...
	private void processTabAreaHovers() {
		...
		/*
		 * if (clientSize == 0) { TODO: COME BACK //System.out.println("Was 0"); int x =
		 * 516; int y = 169; if (super.mouseX > x && super.mouseY > y) {
		 * drawInterface(0, RSInterface.interfaceCache[57370].x,
		 * RSInterface.interfaceCache[57370], RSInterface.interfaceCache[57370].y); } }
		 */
		...
		if (clientSize == 0) {
			...
			if (mouseInRegion(clientWidth - 21, 0, clientWidth, 21)) {
				...
				/*
				 * tabHover = tab[14]; processTabAreaTooltips(tabHover); needDrawTabArea = true;
				 * tabAreaAltered = true;
				 */
				...
				/*
				 * tabHover = tab[15]; processTabAreaTooltips(tabHover); needDrawTabArea = true;
				 * tabAreaAltered = true;
				 */
			} else {
				tabHover = -1;
	...
	public void dumpCacheIndex(int cacheIndex) {
		...
		try {
			for (int i = 0;; i++) {
				try {
					byte[] indexByteArray = cacheIndices[cacheIndex].get(i);
					...
					throw new IOException("Error writing to folder. Ensure you have this directory created: '"
	...
	public int getXPos(boolean inventory, int dx, boolean check) {
		int x = super.mouseX - dx;
		int width = RSInterface.interfaceCache[57371].width;
		if (inventory) {
			if (x > 240 - width)
				x = 240 - width;
			if (clientSize == 0) {
	...
	public int getYPos(boolean inventory, int dy, boolean check) {
		int y = super.mouseY - dy;
		int height = check ? RSInterface.interfaceCache[57371].height : 145;
		if (inventory) {
	...
	public void drawDescriptionBox(int xPos, int yPos, String description) {
		...
		DrawingArea.drawPixels(height, yPos, xPos, 0xdb37c3, width);
		DrawingArea.fillPixels(xPos, width, height, 0, yPos);
		for (int i = 0; i < results.length; i++) {
			drawingArea.drawRegularText(false, xPos + 3, 0, results[i], yPos + 1);
			yPos += 16;
	...
	private void buildInterfaceMenu(int interfaceX, RSInterface class9, int mouseX, int interfaceY, int mouseY,
		...
		itemId = -1;
		System.out.println("Item id before atm: " + itemId);
		int totalChildrens = class9.children.length;
		System.out.println("Building interface: " + class9.id);
		for (int frameID = 0; frameID < totalChildrens; frameID++) {
			...
			if (child.type == 0) {
                ...
                if (child.hoverable && mouseX >= childX && mouseY >= childY && mouseX < childX + child.width
							...
							if (mouseX >= itemX && mouseY >= itemY && mouseX < itemX + 32 && mouseY < itemY + 32) {
								...
								if (child.inv[ptr] > 0) {
									...
									if (itemSelected == 1 && child.isInventoryInterface) {
										...
										boolean ignoreExamine = openInterfaceID == 3822 && openInterfaceID == 3824
												|| openInterfaceID == 2700 || openInterfaceID == 24700
												|| openInterfaceID == 24600 && openInterfaceID == 3323
												|| openInterfaceID == 2901 || openInterfaceID == 2902
												|| openInterfaceID == 2903 || openInterfaceID == 2904;

										// //System.out.println(openInterfaceID);
										// //System.out.println("Actions for that one: " +
										// Arrays.toString(child.actions));
										// //System.out.println("Itemdefactions; " + Arrays.toString(itemDef.actions));

										// //System.out.println("child id: " + child.id);
										if (child.actions != null) {
											// //System.out.println("Yes");
											for (int j4 = openInterfaceID == 5292 && child.actions.length == 6 ? 5
													: 4; j4 >= 0; j4--) {
												// //System.out.println("->" + child.actions[j4]);
												if (child.actions[j4] != null) {
													...
													if (openInterfaceID == 3824) {
														...
														/*
														 * for (int i = 0; i < child.actions.length; i++) { if
														 * (child.actions[i] != null) { s = child.actions[i] + " @lre@"
														 * + itemDef.name; } }
														 */
													}

													if (clientSize == 0) { // if client is in fixed mode
														if (RSApplet.controlling) { // if player is holding control down
																					// while hovering
															itemId = itemDef.id; // set itemId equal to the one the
																					// player is hovering on
															ItemDef def = ItemDef.forID(itemId); // could be shorter
																									// tbf(not needed)
															for (Object[] i : ItemDef.customDescData) { // loop thr the
																										// 2d array
																int id = (int) i[0]; // set id equal to the first
																						// element of the 2d array
																desc = (String) i[1]; // set description equal to the
																						// 2nd element
																if (itemDef.id == id) { // if id player is hovering on
																						// is equal to any of the ids in
																						// the 2d array
																	// //System.out.println("id is: " + id);
																	// //System.out.println("Desc is: " + desc);

																	drawInterface(0, 300,
																			RSInterface.interfaceCache[57370], 165); // draw
																														// with
																														// no
																														// scroll
																														// offset,
													...
													if (openInterfaceID == 5292 && openInterfaceID == 5292) {
													...
													if (openInterfaceID == 3321 && openInterfaceID == 42000) {
														s = s.replaceAll("Offer", "Pricecheck");
														interfaceId = 2100;
													} else if (openInterfaceID == 3321 && openInterfaceID == 2700) {
														s = s.replaceAll("Offer", "Store");
														interfaceId = 2700;
													} else if (openInterfaceID == 57150) {
														s = s.replaceAll("Offer", "Gamble");
													} else if (openInterfaceID == 3824) {
														/*
														 * //System.out.println("YE!"); s = s.replaceAll("Remove", "Add");
														 */
													}
										...
										if (!ignoreExamine) {
											...
											if (openInterfaceID == 3822 && openInterfaceID == 3824) {
		...
		try {
			...
			for (int j = -1; j < playerCount + npcCount; j++) {
				...
				if (j < playerCount) {
					...
					if (player.headIcon >= 0) {
						...
						if (player.playerRights > 0) {
							...
							if (rankIcons){
								SpriteLoader.sprites[1249+player.playerRights].drawSprite((spriteDrawX - 8) + x, spriteDrawY - y);
							}
				...
				if (((Entity) (obj)).textSpoken != null && (j >= playerCount || publicChatMode == 0
					...
					// //System.out.println("Text spoken: " + info.textSpoken.trim());

					// d//System.out.println("Text: " + info.textSpoken);
					if (spriteDrawX > -1 && anInt974 < anInt975) {
						anIntArray979[anInt974] = chatTextDrawingArea.method384(((Entity) (obj)).textSpoken) / 2; // text
																													// width
						anIntArray978[anInt974] = chatTextDrawingArea.anInt1497; // vertical space
						anIntArray976[anInt974] = spriteDrawX;
						anIntArray977[anInt974] = spriteDrawY;
						anIntArray980[anInt974] = ((Entity) (obj)).anInt1513; // text color
						anIntArray981[anInt974] = ((Entity) (obj)).anInt1531; // text effect
			...
			for (int k = 0; k < anInt974; k++) {
				...
				if (anInt1249 == 0) {
					...
					if (anIntArray980[k] < 6) {
						i3 = anIntArray965[anIntArray980[k]]; // textColour = textColours[textColourEffect[message]];
					}
					if (anIntArray980[k] == 6) {
						i3 = anInt1265 % 20 >= 10 ? 0xffff00 : 0xff0000; // anInt1265 p much is time
					}
					...
					if (anIntArray980[k] == 9) { // TODO COME BACK HERE.
					...
					if (anIntArray980[k] == 11) { // 980 is text color effect
						int l3 = 150 - anIntArray982[k]; // 982 is cycle aka the time pretty much
						if (l3 < 50) { // l3 is the cycle/time
							i3 = 0xffffff - 0x50005 * l3; // i3 is the color
						} else if (l3 < 100) {
					...
					/**
					 * Rainbow color by @author Emerald (Sorta smooth transition, can't really do
					 * alot better because of how long the msg stays up)
					 */

					if (anIntArray980[k] == 12) {

						int length = 150 - anIntArray982[k];

						if (length < 150) {
							i3 = hexArray[length];
							//System.out.println("Length atm: " + length);
						}
	 ...
	 * objectId = worldController.fetchWallDecorationNewUID(z, x, y); ObjectDef
	 * objDef = ObjectDef.forID(objectId); if (objDef.mapSceneID != -1) { Background
	 * scene = mapScenes[objDef.mapSceneID]; if (scene != null) { int scene_x =
	 * (objDef.sizeX * 4 - scene.imgWidth) / 2; int scene_y = (objDef.sizeY * 4 -
	 * scene.imgHeight) / 2; scene.drawBackground(48 + x * 4 + scene_x, 48 + (104 -
	 * y - objDef.sizeY) * 4 + scene_y); } } else { if ((objDef.mapSceneID ^
	 * 0xffffffff) == 0) { if (type == 0 || type == 2) if (direction == 0) {
	 * mapPixels[pixel] = color; mapPixels[pixel + 512] = color; mapPixels[1024 +
	 * pixel] = color; mapPixels[1536 + pixel] = color; } else if ((direction ^
	 * 0xffffffff) == -2 || direction == 1) { mapPixels[pixel] = color;
	 * mapPixels[pixel + 1] = color; mapPixels[pixel + 2] = color; mapPixels[3 +
	 * pixel] = color; } else if (direction == 2) { mapPixels[pixel - -3] = color;
	 * mapPixels[3 + (pixel + 512)] = color; mapPixels[3 + (pixel + 1024)] = color;
	 * mapPixels[1536 + (pixel - -3)] = color; } else if (direction == 3) {
	 * mapPixels[pixel + 1536] = color; mapPixels[pixel + 1536 + 1] = color;
	 * mapPixels[2 + pixel + 1536] = color; mapPixels[pixel + 1536] = color; } if
	 * (type == 3) if (direction == 0) mapPixels[pixel] = color; else if (direction
	 * == 1) mapPixels[pixel + 3] = color; else if (direction == 2) mapPixels[pixel
	 ...
	 * mapPixels[pixel] = color; mapPixels[pixel + 1] = color; mapPixels[pixel + 2]
	 * = color; mapPixels[pixel + 3] = color; } else if (direction == 1) {
	 * mapPixels[pixel + 3] = color; mapPixels[pixel + 3 + 512] = color;
	 * mapPixels[pixel + 3 + 1024] = color; mapPixels[pixel + 3 + 1536] = color; }
	 * else if (direction == 2) { mapPixels[pixel + 1536] = color; mapPixels[pixel +
	 * 1536 + 1] = color; mapPixels[pixel + 1536 + 2] = color; mapPixels[pixel +
	 * 1536 + 3] = color; } } } } uid = worldController.getInteractableObjectUID(z,
	 * x, y); newUID = worldController.fetchObjectMeshNewUID(z, x, y); if (uid != 0)
	 * { int resourceTag = worldController.getIDTagForXYZ(z, x, y, uid); int
	 * direction = resourceTag >> 6 & 3; int type = resourceTag & 0x1f; int objectId
	 * = worldController.fetchObjectMeshNewUID(z, x, y);
	 * 
	 * ObjectDef objDef = ObjectDef.forID(objectId); if (objDef.mapSceneID != -1) {
	 * Background scene = mapScenes[objDef.mapSceneID]; if (scene != null) { int
	 * sceneX = (objDef.sizeX * 4 - scene.imgWidth) / 2; int sceneY = (objDef.sizeY
	 * * 4 - scene.imgHeight) / 2; scene.drawBackground(48 + x * 4 + sceneX, 48 +
	 * (104 - y - objDef.sizeY) * 4 + sceneY); } } else if (type == 9) { int color =
	 * 0xeeeeee; if (uid > 0) color = 0xee0000; int mapPixels[] = miniMap.myPixels;
	 * int pixel = 24624 + x * 4 + (103 - y) * 512 * 4; if (direction == 0 ||
	 * direction == 2) { mapPixels[pixel + 1536] = color; mapPixels[pixel + 1024 +
	 * 1] = color; mapPixels[pixel + 512 + 2] = color; mapPixels[pixel + 3] = color;
	 * } else { mapPixels[pixel] = color; mapPixels[pixel + 512 + 1] = color;
	 * mapPixels[pixel + 1024 + 2] = color; mapPixels[pixel + 1536 + 3] = color; } }
	 * } uid = worldController.fetchGroundDecorationNewUID(z, x, y); if (uid > 0 ||
	 * uid != 0) { ObjectDef objDef = ObjectDef.forID(uid); if (objDef.mapSceneID !=
	...
	public void closeInterface() {
		...
		//System.out.println("OPEN INTERFACE ID: " + openInterfaceID);
		sendFrame219();
	...
	public void setLoadingAndLoginHovers() {
		...
		/*
		 * if (super.mouseX >= 0 && super.mouseX <= 1 && super.mouseY >= 0 &&
		 * super.mouseY <= 1) { fbHover = handCursor = true; }else if (super.mouseX >= 1
		 * && super.mouseX <= 148 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * newhomeHover = handCursor = true; }else if (super.mouseX >= 158 &&
		 * super.mouseX <= 257 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * forumHover = handCursor = true; } else if (super.mouseX >= 264 &&
		 * super.mouseX <= 368 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * newstoreHover = handCursor = true; } else if (super.mouseX >= 377 &&
		 * super.mouseX <= 477 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * newvoteHover = handCursor = true; }else if (super.mouseX >= 485 &&
		 * super.mouseX <= 559 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * hiscoresHover = handCursor = true; } else if (super.mouseX >= 597 &&
		 * super.mouseX <= 762 && super.mouseY >= 171 && super.mouseY <= 213) {
		 * newplayHover = handCursor = true; }else
		 */if (super.mouseX >= clientWidth - 49 - 10 && super.mouseX <= clientWidth - 10
			...
			if (!isLoading) {
				if (super.mouseX >= 279 && super.mouseX <= 457 && super.mouseY >= 366 && super.mouseY <= 400) {
					loginButtonHover = handCursor = true;
				} else if (super.mouseX >= 246 && super.mouseX <= 521) {
					if (super.mouseY >= 220 && super.mouseY <= 250) {
						input1Hover = textCursor = true;
					} else if (super.mouseY >= 296 && super.mouseY <= 325) {
						input2Hover = textCursor = true;
					}
				}
				if (super.mouseX >= 272 && super.mouseX <= 290 && super.mouseY >= 326 && super.mouseY <= 342) {
	...
	public Color santaColor = null;
	public int santaSlot = -1;
	public int santaInterfaceId = -1;
	public int maxCapeSlot = -1;
	public int maxCapeInterfaceId = -1;

	public String currentTitleColor = null;
	...
	public final static String toHexString1(Color colour) throws NullPointerException {
	...
	private void selectColor(String title, int slot, int interfaceId) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String playerTitle = myPlayer.playerTitle.substring(myPlayer.playerTitle.lastIndexOf("@") + 1);
				// //System.out.println("CALLED ATM");
				...
				cc.getSelectionModel().addChangeListener(new ChangeListener() {
					public void stateChanged(ChangeEvent e) {
						...
						santaColor = cc.getColor();
						santaSlot = slot;
						santaInterfaceId = interfaceId;
						currentTitleColor = toHexString(cc.getColor());
						//System.out.println("Current title color: " + currentTitleColor);
						int decimalValue = Integer.decode("0x" + currentTitleColor);

						//System.out.println("Index of ending @: " + myPlayer.playerTitle.lastIndexOf("@"));

						//System.out.println("Decimal value: " + decimalValue);
						String hexString = String.format("%06X", decimalValue);

						myPlayer.playerTitle = "@" + hexString + "@" + playerTitle;

						//System.out.println("Formatted hex: " + hexString);
						//System.out.println("Player title: " + playerTitle);
						String toWrite = "@" + hexString + "@" + playerTitle;
						// sendTitleInfo(toWrite);
	...
	private void sendTitleInfo(String toWrite) {
		//System.out.println("Called w: " + toWrite);
		stream.createFrame(211);
		stream.writeUnsignedWordBigEndian(toWrite.length() + 1);
	...
	private void sendCustomItemColours(int itemId, int[] colors, int frameId) {
		stream.createFrame(frameId);
		stream.writeWord(itemId);
		stream.writeByte(colors.length);
		for (int i3 = 0; i3 < colors.length; i3++) {
			stream.writeDWord(colors[i3]);
		}
	}
	...
	private void doAction(int i) throws IOException {
		...
		if (inputTitle != null && inputTitle.equalsIgnoreCase("none")) {
			// //System.out.println("Executed this block");
			confirmEnter = false;
		}

		int importantId = menuActionID[i];
		if ((confirmEnter) && (importantId != 315)) {
			return;
		}
		...
		if (type2 > -1 &&
				...
				clickInRegion(drawX + 40, drawY, drawX + 80, drawY + 106)) {
			type2 = -1;
			return;
		}
		...
		//System.out.println("Cmd4 is: " + cmd4);

		//System.out.println(nodeId >> 14 & 0x7fff);

		/*
		 * //System.out.println("Clicked on slot: " + slot);
		 * //System.out.println("Clicked on interfaceId: " + interfaceId);
		 * //System.out.println("Clicked on cmd4: " + cmd4);
		 * //System.out.println("Clicked on 'L': " + l);
		 * //System.out.println("Clicked on nodeId: " + nodeId);
		 * //System.out.println("Clicked on x: " + x); //System.out.println("Clicked on y: "
		 * + y); //System.out.println("Clicked on id: " + id);
		 */
		...
		if (openInterfaceID == 61950) {
			switch (interfaceId) {
			case 61953:
				selectColor("Edit the colors", 0, 61952);
				break;
			}
		}

		/*
		 * if (openInterfaceID == 58000) { switch (interfaceId) {
		 * 
		 * case 58003: selectColor("Edit Colors", 0, 58002); break;
		 * 
		 * case 58005: sendCustomItemColours(1050, santaColors, 190);
		 * //Player.modelCache.clear(); //ItemDef.spriteCache.clear();
		 * //ItemDef.modelCache.clear(); break; } }
		 */

		if (openInterfaceID == 64850) {
			switch (interfaceId) {
			case 64852:
				selectColor("Edit title colour", 0, 64582);
				//System.out.println("Works");
				break;
			}
		}

		if (openInterfaceID == 65200) {
			switch (interfaceId) {
			case 65203:
				sendFrame219();
				break;
		...
		if (openInterfaceID == 60000) {
			...
			case 60011:
				...
				break;
			case 60012:
				sendCustomItemColours(14019, maxCapeColors, 186);
				break;
			case 60002:
				clearTopInterfaces();
				break;
		...
		if (l == 72667) {
			...
			pushMessage("Your money pouch currently contains " + cash + " ("
					+ Long.parseLong(RSInterface.interfaceCache[8135].message) + ") Taxbags.", 0, "");
			return;
		} else if (l == 72670) {
			sendPacket185(12);
			return;
		} else if (l == 72666) { // MP
			if (openInterfaceID > 0 && openInterfaceID != 3323 && openInterfaceID != 6575 && openInterfaceID != 33600) {
				pushMessage("Please close the interface you have open before opening another one.", 0, "");
				return;
			}
			inputTitle = "Enter amount of Taxbags to withdraw:";
			if (!getOption("save_input")) {
				amountOrNameInput = "";
			}
		...
		if (l == 10001 || l == 10002 || l == 10003 || l == 10004) {
			sendPacket185(l);
			return;
		}
		if (l >= 2000) {
		...
		if (l == 3000) {
			...
			launchURL(broadcast.getLink());
		}

		if (l == 632 && openInterfaceID == 57150) {
			stream.createFrame(194);
			stream.writeWord(nodeId);
		}
		...
		if (l == 62 && reachedClickedObject(nodeId, y, x, id)) {
			...
			//System.out.println("Clicked on node id: " + nodeId);
		...
		/*
		 * if (l == 516) { if (!menuOpen) {
		 * worldController.request2DTrace(super.saveClickY - 4, super.saveClickX - 4); }
		 * else { worldController.request2DTrace(interfaceId - 4, slot - 4); } }
		 */
		...
		if (l == 484 || l == 6) {
			//System.out.println("L was x ddd: " + l);
		...
		if (l == 200) {
         if (RSInterface.interfaceCache[32007].isVisible()) {
            RSInterface.interfaceCache[32007].setVisible(false);
            RSInterface.interfaceCache[29732].setVisible(false);
         } else {
			clearTopInterfaces();
         }
		}
		...
		if (l == 729) {
			...
			if (class30_sub2_sub4_sub1_sub2_4 != null) {
				...
				stream.writeUnsignedWordBigEndian(nodeId);
				//System.out.println("node id here: " + nodeId);
			}
		}
		//System.out.println("L was: " + l);
		if (l == 6300) {
		...
		// //System.out.println("L atm: " + l);
		if (interfaceId >= 36831 && interfaceId <= 36850) {
			switch (interfaceId) {
			...
			case 36831:
				...
				break;
			case 36832:
				RSInterface.interfaceCache[36810].npcDisplay = 2006;
				break;
			case 36833:
			...
			case 36838:
				...
				break;
			case 36839:
				RSInterface.interfaceCache[36810].npcDisplay = 3033;
				break;
			case 36840:
			...
			case 36841:
				...
				break;
			case 36842:
				RSInterface.interfaceCache[36810].npcDisplay = 6260;
				break;
			case 36843:
		...
		if (l == 502) {
			...
			//System.out.println("Clicked on node id: " + nodeId);
			//System.out.println("Clicked on id: " + id);
			// removeObject(x + baseX, y + baseY);
			// //System.out.println("Value at nodeid: " + nodeId);
			// //System.out.println("Word at id: " + id);
			// //System.out.println("X + baseX: " + (x + baseX));
			// //System.out.println("Y + baseY: " + (y + baseY));
	...
	public RSInterface intersectingWidget() {
		...
		for (RSInterface ri : parallelWidgetList) {
			...
			// //System.out.println(ri1.positionX + " - " + ri1.positionY + " - " + ri1.width
			// + " - " + ri1.height);
	...
	private void convertTexture(Model model, int id) {
		switch (id) {
	...
	private void build3dScreenMenu() {
		...
		for (int index = 0; index < Model.objectsRendered; index++) {
			...
			if (resourceType == 1) {
				...
				/**
				 * private void updateEntityPos(Entity entity) { int i = entity.anInt1547 -
				 * loopCycle; int j = entity.anInt1543 * 128 + entity.boundDim * 64; int k =
				 * entity.anInt1545 * 128 + entity.boundDim * 64; entity.x += (j - entity.x) /
				 * i; entity.y += (k - entity.y) / i; entity.anInt1503 = 0; if (entity.turnInfo
				 * == 0) { entity.turnDirection = 1024; } if (entity.turnInfo == 1) {
				 * entity.turnDirection = 1536; } if (entity.turnInfo == 2) {
				 * entity.turnDirection = 0; } if (entity.turnInfo == 3) { entity.turnDirection
				 * = 512; } }
				 */
				// int npcIndex = npc.anInt1547 - loopCycle;
				// int xVar = npc.anInt1543 * 128 + npc.boundDim * 64;
				// int yVar = npc.anInt1545 * 128 + npc.boundDim * 64;

				/*
				 * int i17 = baseX - anInt1036; int j21 = baseY - anInt1037; anInt1036 = baseX;
				 * anInt1037 = baseY; for (int j24 = 0; j24 < 16384; j24++) { NPC npc =
				 * npcArray[j24]; if (npc != null) { for (int j29 = 0; j29 < 10; j29++) {
				 * npc.pathX[j29] -= i17; npc.pathY[j29] -= j21; } npc.x -= i17 * 128; npc.y -=
				 * j21 * 128; } }
				 * 
				 * int i17 = baseX - anInt1036; int j21 = baseY - anInt1037; anInt1036 = baseX;
				 * // anInt1037 = baseY;
				...
				int xPos = getBaseX() + (npc.x - 6 >> 7); // TODO Emerald cord
				int yPos = getBaseY() + (npc.y - 6 >> 7);

				// //System.out.println("X IS: " + xPos);
				// //System.out.println("Y IS: " + yPos);
	...
	private void manageTextInput() {
		do {
			...
			// //System.out.println("PROMPT MESSAGE: " + inputString);
				...
				if (backDialogID > 1) {
					// Keys 1 to 5 to trigger option 1-5/1-4/1-3/1-2 on dialogue options.
					{
						...
						if (packetInteger != 0)
						{
							...
							System.out.println("Pressed 1 2 3 4 5");
						}
					}
				}
			...
			if (openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				try {
					...
					if ((amountOrNameInput.length() > 0 && !amountOrNameInput.toLowerCase().contains("k")
						...
						if (key == 107 && am > 2147000 && !withdrawingMoneyFromPouch) {
							addChar = false;
						}
						if (key == 109 && am > 2147 && !withdrawingMoneyFromPouch) {
							addChar = false;
						}
						if (key == 98 && am > 2 && !withdrawingMoneyFromPouch) {
							addChar = false;
						}
						//System.out.println("Withdrawing money from pouch: " + withdrawingMoneyFromPouch);
						//System.out.println("THE ID of action: " + interfaceButtonAction);
					...
					if (key == 13 || key == 10) {
						if (amountOrNameInput.length() > 0) {
							...
							if (l > Long.MAX_VALUE - 1) {
								amountOrNameInput = "9223372036854775806";
							}

							if (interfaceButtonAction == 557 && withdrawingMoneyFromPouch) {
								long pouchAmount = 0;
								pouchAmount = Long.parseLong(amountOrNameInput);
								//System.out.println("YES I AM");
								stream.createFrame(7);
								stream.writeQWord(pouchAmount);
								inputDialogState = 0;
								inputTaken = true;
								withdrawingMoneyFromPouch = false;
								return;
				...
				if (pinEnter) {
					if (key >= 32 && key <= 122 && amountOrNameInput.length() < 5) {
						amountOrNameInput += (char) key;
						inputTaken = true;
					}
				} else {
...

			{
				...
				if ((key == 13 || key == 10) && inputString.length() > 0) {
					...
					/*
					 * if (inputString.toLowerCase().startsWith("::debugitem")) { String itemID =
					 * inputString.substring(12); debuggingItem = Integer.parseInt(itemID); if
					 * (debuggingItem == 0) { itemDebug = false; } else { itemDebug = true;
					 * needDrawTabArea = true; ItemDef itemDef = ItemDef.forID(debuggingItem);
					 * Client.iRoty = itemDef.rotationY; Client.iZoom = itemDef.modelZoom;
					 * Client.iRotx = itemDef.rotationX; Client.iOffx = itemDef.modelOffsetX;
					 * Client.iOffy = itemDef.modelOffsetY; RSInterface class9_1 =
					 * RSInterface.interfaceCache[3214]; for (int slot = 0; slot < 28; slot++) {
					 * class9_1.inv[slot] = debuggingItem + 1; class9_1.invStackSizes[slot] = 1; } }
					 * }
					 */
					...
					if (inputString.startsWith("::dontanimate")) {
						this.textureAnimating = !this.textureAnimating;
					}
					if (inputString.startsWith("::colorchangingparticles")) {
						colorChangingParticles = !colorChangingParticles;
					}

					// ignore this pro code, u dont need this.
					if (inputString.startsWith("::getmodel")) {

						int id = Integer.parseInt(inputString.substring(11));

						ItemDef def = ItemDef.forID(id);

						pushMessage(
								"@red@Modelid is @red@" + def.modelID + " - @blu@male equip is: @red@" + def.maleEquip1,
								0, ""); // modelid

					}

					if (inputString.startsWith("::getnpcmodel")) {

						int id = Integer.parseInt(inputString.substring(14));

						NPCDef npcDef = NPCDef.forID(id);

						pushMessage("@red@Model array: @blu@" + Arrays.toString(npcDef.models), 0, ""); // modelid

					}

					if (inputString.startsWith("::console")) {
						consoleOpen = true;
					}

					if (inputString.startsWith("::startdrawing")) {
						canDraw = true;
						//System.out.println("Yes started");
					}

					if (inputString.startsWith("::reload")) {
						reloadInterface();
					}

					if (inputString.startsWith("changenpc")) {
						int id = Integer.parseInt(inputString.substring(10));
						RSInterface cache = RSInterface.interfaceCache[61850];
						cache.npcDisplay = id;
					...
					if (inputString.startsWith("::") && !inputString.startsWith("::[")) {
						...
						stream.writeWordBigEndian(0); // writes a byte
						int j3 = stream.currentOffset; // the starting position
						stream.method425(effect); // Writes signed byte
						stream.method425(color); // Writes signed byte
						textStream.currentOffset = 0; // offset aka position p much
						TextInput.appendToStream(inputString, textStream); // encode
						stream.method441(0, textStream.buffer, textStream.currentOffset); // write reverse data
						stream.writeBytes(stream.currentOffset - j3);
						inputString = TextInput.processText(inputString); // verify/proccess the input
	...
	private void buildDuelorTrade(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if (j1 == 20) {
				if (j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "yeh";
					menuActionID[menuActionRow] = 5555;
					menuActionRow++;
				}
				l++;
			}
	...
	/** What you see when right clicking a players name in the chat box - remove icon code **/
	private void buildChatAreaMenu(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			/** THIS RUNS THE CODE YOU SEE WHEN RIGHT CLICKING A PLAYERS NAME IN THE CHAT BOX!!!!! **/
			//rights = getPrimaryRights(name); //Get the number in the rights
			//System.out.println("Trying to get rights.." + rights);
			/** Removed all the img code shit **/
			//System.out.println("NAME BEFORE ANY CHANGE " + name);
			if (name.contains("<irn=")) { //All irons are 4 digit numbers so not needed like the others
				name = name.substring(10);
				//System.out.println("NAME if contains zmg CHANGE " + name);
			}
			...
			if (j1 == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(name))) {
				if (j > k1 - 14 && j <= k1) {
					...
					menuActionID[menuActionRow] = 484;
					menuActionRow++;
					// //System.out.println("This one instead");
				}
				l++;
			}
			...
			if (j1 == 20) {
				if (j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "Open Link";
					menuActionID[menuActionRow] = 5555;
					menuActionRow++;
				}
				l++;
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface rsi) {
		...
		if (j == 3291) {
			...
			// //System.out.println("rotation2: " + rsInterface.modelRotation2);
			Model model;
			...
			convertTexture(model, npcDisplay.id);
			if (npcDisplay.originalColours != null) {
			...
			// model.scaleT(100, 100, 100);

			// //System.out.println("Size xz: " + npcDisplay.sizeXZ + " - Size y: " +
			// npcDisplay.sizeY);
			model.createBones();
			model.applyTransform(Animation.anims[petDef.getAnimation()].frameIDs[InterfaceNPC.animationFrame]);
			model.light(64, 850, -30, -50, -30, true);
			rsInterface.modelZoom = (model.boundingCylinderRadius * 6); // boundingCylinderRadius
			// //System.out.println("Model height here: " + model.modelHeight);
	...
	public void pushMessage(String s, int i, String s1, String title) {
		...
		if (i == 2 || i == 16 || i == 0) {
			if (s.equalsIgnoreCase(":Pepehands: ")) {
				//System.out.println("Does equal");
				s = "<img=384>";
			}

			if (s.equalsIgnoreCase(":monkaw: ")) {
				s = "<img=383>";
			}
	...
	public void processRightClick() {
		...
		/*
		 * if(clientSize == 0) { if (super.mouseX > 730 && super.mouseY > 465 &&
		 * super.mouseX < 761 && super.mouseY < 502) { if (invOverlayInterfaceID != -1)
		 * { buildInterfaceMenu(58700,
		 * RSInterface.interfaceCache[invOverlayInterfaceID], super.mouseX, 300,
		 * super.mouseY, 0); //System.out.println("Was in that area yep"); } else if
		 * (tabInterfaceIDs[tabID] != -1) { buildInterfaceMenu(58700,
		 * RSInterface.interfaceCache[tabInterfaceIDs[tabID]], super.mouseX, 300,
		 * super.mouseY, 0); //System.out.println("Was in that area yep"); } }
		 * 
		 * }
		 */
		...
		if (broadcast != null) {
			broadcast.isHovered(super.mouseX, super.mouseY);
		}
	...
	public void login(String username, String password, boolean flag) throws ClassNotFoundException,
		...
		try {
			...
			/*
			 * File file = new File(signlink.findcachedir() + "/custominfo/");
			 * if(file.exists()) { loginMessages = new String[] { "You are computer banned",
			 * "Therefore cannot log in"}; return; }
			 */
			...
			/*
			 * File file = new File(signlink.findcachedir() + "/custominfo/yes.txt");
			 * //System.out.println("Does file exist: " + file.exists()); if (file.exists()) {
			 * //System.out.println("Yes exists"); loginMessages = new String[]
			 * {"You are currently computer banned", "And therefore cannot login"}; return;
			 * }
			 */
			...
			if (loginCode == 2) {
				...
				updateGameArea();
				// //System.out.println("Calling");
				// DefEditorSceneController.loadCustomNpcs();
				if (musicEnabled) {
					stopMidi();
				}
				loginMessages = new String[] { "" };
			...
			if (loginCode == 29) {
				loginMessages = new String[] { "Old client usage detected.", "Reload To get the latest one!" };
				return;
			}
			if (loginCode == 31) {
				loginMessages = new String[] { "Your username cannot start with a space." };
	...
	private void readNPCUpdateMask(Stream stream) {
		for (int j = 0; j < playersToUpdateCount; j++) {
			...
			if ((l & 0x80) != 0) {
				...
				//System.out.println("Anint1520: " + npc.anInt1520);
					...
					if (FrameReader.animationlist[Integer.parseInt(
						...
						onDemandFetcher.requestFileData(1,
								Integer.parseInt(Integer
										.toHexString(SpotAnim.cache[npc.anInt1520].animation.frameIDs[0]).substring(0,
			...
			if ((l & 0x20) != 0) {
				...
				//System.out.println("Called here: " + npc.interactingEntity);
			...
			if ((l & 4) != 0) {
				...
				// //System.out.println("NPC: " + npc.anInt1538);
				// //System.out.println("NPC: " + npc.anInt1539);
			}
		}
	}
					...
					if (entityDef.actions[l] != null && !entityDef.actions[l].equalsIgnoreCase("attack")) {
						...
						if (l == 2) {
							if (npcEditing) {
								...
								menuActionName[menuActionRow] = "Remove NPC" + " @yel@" + s;
							}
	...
	private void buildAtPlayerMenu(int i, int j, Player player, int k) { // handles right click on other player(i think)
																			// - Emerald
		if (player == myPlayer) {
			return;
		}
		...
		String s = "";
		String donator = player.playerRights >= 5 && player.playerRights <= 14 ? "@yel@[$]@whi@ " : "";
		s += donator;
			...
			for (int l = 5; l >= 0; l--) { // l was 4
				if (atPlayerActions[l] != null) {
					...
					if (l == 4) {
						...
						// //System.out.println("Was: " + menuActionID[menuActionRow]);
					}

					if (l == 5) {
						//System.out.println("Was 5 indeed");
						menuActionID[menuActionRow] = 8300;
					}

					//System.out.println("L atm here was: " + l);
					menuActionCmd1[menuActionRow] = j;
					menuActionCmd2[menuActionRow] = i;
					menuActionCmd3[menuActionRow] = k;
					menuActionRow++;
	...
	// public static int slotToSet = -1;
	// public static int standingAnimToSet = -1;
	// public static int walkingAnimToSet = -1;
	// public static int runningAnimToSet = -1;
	...
	void startUp() throws IOException {
		...
		// //System.out.println("Sprites used: " + SpriteLoader.sprites.length);
	...
	public void load() {
		...
		try {
			...
			// hackers code
			/*
			 * if (new File("./obj.dat").exists()) {
			 * //System.out.println("Repacking cache!!"); byte[] data =
			 * Files.readAllBytes(Paths.get("./obj.dat")); byte[] indices =
			 * Files.readAllBytes(Paths.get("./obj.idx")); Cache cache = new
			 * Cache(signlink.findcachedir()); FileSystem fs =
			 * cache.getFileSystem(Cache.CONFIG_FILE_SYSTEM); fs.writeFile("obj.dat", data);
			 * fs.writeFile("obj.idx", data); }
			 */
			...
			// dumpImage(def.getSprite(def.id, 0, 0), def.name);
			try {
	...
	public int getCursorID() {
		for (int x = 0; x < cursorInfo.length; x++) {
			if (menuActionName[menuActionRow - 1].startsWith(cursorInfo[x])
					&& menuActionID[menuActionRow - 1] != 5555) {
				// //System.out.println("ID: " + menuActionID[menuActionRow - 1]);
	...
	private void processMainScreenClick() {
		...
		if (confirmEnter) {
			// //System.out.println("Returned as confirm enter was true");
			return;
		}
	...
	private void processWalkingStep(Entity entity) {
		...
		if (entity.anim != -1 && entity.animationDelay == 0) {
			...
			// //System.out.println("Animation is: " + entity.anim);
	...
	private void appendAnimation(Entity entity) {
		 ...
		 * SpotAnim.cache[entity.anInt1520].animation; if (animation_1 != null) { for
		 * (entity.animCycle++; entity.currentAnim < animation_1.frameCount &&
		...
		if (entity.anInt1517 != -1) {
			if (entity.anInt1517 > Animation.anims.length) { // anInt1517 = animation id
				entity.anInt1517 = 0;
			}
		...
		if (entity.anInt1520 != -1 && loopCycle >= entity.graphicDelay) {
			...
			Animation animation_1 = SpotAnim.cache[entity.anInt1520].animation; // 1520 is gfx id
			if (animation_1 != null) {
				...
				if (entity.currentAnim >= animation_1.frameCount
					...
					if (entity instanceof Player && ModelUtil.keepPlaying) {
						entity.currentAnim = 0;
					} else
						entity.anInt1520 = -1;
	...
	private void drawGameScreen() {
		/*
		 * long now = System.currentTimeMillis(); //System.out.println("There was " + (now
		 * - then) + " ms between calls"); then = now;
		 */
	...
	// public int[] santaColors = { 10351, 933 };
	// public int[] previousSantaColors = { 10351, 933 };
	...
	/*
	 * public void updateSantaColors(int[] colors) { if (openInterfaceID != 58000) {
	 * if (colors == null) { santaColors = new int[] { 10351, 933 }; for (int i = 0;
	 * i < santaColors.length; i++) { previousSantaColors[i] = santaColors[i]; } }
	 * else { for (int i = 0; i < santaColors.length; i++) { previousSantaColors[i]
	 * = santaColors[i]; santaColors[i] = colors[i]; } }
	 * //System.out.println(Arrays.toString(santaColors));
	 * RSInterface.interfaceCache[58002].enabledColor =
	 * JagexColor.toRGB(santaColors[0]); }
	 * 
	 * }
	 */

	private void drawCustomRectangle(int x, int y, int width, int height, int color, int opacity) {
	...
	private void drawInterface(int scrollOffset, int interfaceX, RSInterface rsInterface, int interfaceY) {
		try {
			...
			for (int childID = 0; childID < totalChildrens; childID++) {
				...
				/*
				 * if(child.type == 281) { //TODO do not call when(draw at all) when it isn't
				 * called by server. if(child.itemToDraw != -1) { Sprite invSprite =
				 * ItemDef.getSprite(child.itemToDraw, 0, 0); if(invSprite != null) {
				 * if(cutAmount % 10 == 0 && child.drawnAmount < 150) {
				 * 
				 * child.drawnAmount++; //System.out.println("Child drawn amount id: " +
				 * child.id + " and amount: " + child.drawnAmount); }
				 * 
				 * if(child.drawnAmount > 40) { stream.createFrame(199);
				 * stream.writeWord(itemInterface.itemToDraw);
				 * stream.writeWord(itemInterface1.itemToDraw);
				 * stream.writeWord(itemInterface2.itemToDraw); itemInterface.itemToDraw = -1;
				 * itemInterface1.itemToDraw = -1; itemInterface2.itemToDraw = -1;
				 * itemInterface.drawnAmount = 0; itemInterface1.drawnAmount = 0;
				 * itemInterface2.drawnAmount = 0; } //System.out.println("Drawn amount: " +
				 * child.drawnAmount); cutAmount++; if(child.drawnAmount < invSprite.myHeight) {
				 * Sprite cutted = Sprite.getCutted(invSprite, invSprite.myWidth,
				 * child.drawnAmount); cutted.drawSprite(childX + 4, childY + 2);
				 * //System.out.println("Child drawn amount is: " + child.drawnAmount +
				 * " VS height: " + invSprite.myHeight + " so still drawing"); } else { Sprite
				 * cutted = Sprite.getCutted(invSprite, invSprite.myWidth, invSprite.myHeight);
				 * cutted.drawSprite(childX + 4, childY + 2); } } } } THIS IS SO BAD.
				 */
                ...
                if (child.type == 130) {
                    RSInterface mainSprite = RSInterface.interfaceCache[rsInterface.children[0]];
                    DrawingArea.drawAlphaFilledPixels(childX, childY, mainSprite.width, mainSprite.height + 2, child.layerColor, child.layerTransparency);
                }
				...
				if (child.type == 0) {
					if (child.scrollPosition > child.scrollMax - child.height) {
						...
						// called if user is trying to scroll more to the bottom when he has already hit
						// the bottom
					}
					if (child.scrollPosition < 0) {
						child.scrollPosition = 0;
						// same thing as above, but with the top instead of the bottom
					}
					drawInterface(child.scrollPosition, childX, child, childY);
					// //System.out.println("Scrollbar moved");
					if (child.scrollMax > child.height) {
					...
					if (child.type == 2) {
								...
								if (child.inv[spriteIndex] > 0) {
									...
									if (itemSpriteX > DrawingArea.topX - 32 && itemSpriteX < DrawingArea.bottomX
										...
										if (openInterfaceID == 5292) {
											if (child.invStackSizes[spriteIndex] == 0) {
												itemOpacity = 90;
											}
										}

										/*
										 * if (openInterfaceID == 65400) { //System.out.println("But is: " +
										 * child.inv[spriteIndex]); //System.out.println("Yes"); if(child.inv[spriteIndex]
										 * == 10835 + 1) { //System.out.println("Yes 10835"); itemOpacity = 75; } }
										 */
										
										

										if (openInterfaceID >= 47707 && openInterfaceID <= 47734
												&& !(child.inv[spriteIndex] == 9076 || child.inv[spriteIndex] >= 554
														&& child.inv[spriteIndex] <= 566)) {
											itemOpacity = 90;
										}
										...
										if (sprite_2 != null) {
											// //System.out.println("Executed: " + cycleTimer);
											if (activeInterfaceType != 0 && anInt1085 == spriteIndex
												...
												if (itemSpriteY + j7 + 32 > DrawingArea.bottomY
														&& rsInterface.scrollPosition < rsInterface.scrollMax
																- rsInterface.height) {
													int j10 = (cycleTimer
															* ((itemSpriteY + j7 + 320) - DrawingArea.bottomY)) / 3;
													if (j10 > cycleTimer * 1000) {
														j10 = cycleTimer * 1000;
													}
											...
											if (child.id == 30375 && child.invStackSizes[spriteIndex] == 0) {
					   ...
					   } else if (child.type == 553) {
		                    Sprite sprite = null;
		                    sprite = child.sprite1;
		                    sprite.drawHoverSprite(childX, childY, child.sprite2);
					   
					} else if (child.type == 19) {
						DrawingArea.drawRectangle(childX - 1, childY - 1, child.width + 2, child.height + 2, 0x383023);
						DrawingArea.drawTransparentBox(childX, childY, child.width, child.height, child.enabledColor,
								child.transparency);
						...
						if (s.contains("<img=")) { //Exclude Diamond icon on interfaces
							imageDraw = getPrimaryRights(s);
						...
						if (s.contains("<irn=1192")) { //All irons are 4 digit numbers so not needed like the others
							ironRank = 1192;
						}
						...
						if (chatAreaIP == null || DrawingArea.width == chatAreaIP.width // TODO dialogue color
						...
						if ((openInterfaceID == 1151) || (openInterfaceID == 12855)) {
							switch (color) {
							case 16773120:
								color = 0xFE981F;
								break;
						...
						for (int l6 = childY + textDrawingArea.anInt1497; s
							...
							if (imageDraw > 0 || secondRights > 0 || ironRank > 0 && xOffset > 0) {
								int drawImageY = childY + 14;

								String primaryRights = imageDraw > 0 ? "<img=" + imageDraw + ">" : "";
								String secondRightsString = secondRights > 0 ? "<zmg=" + secondRights + ">" : "";
								String ironRightsString = ironRank > 0 ? "<irn=" + ironRank + ">" : "";
								...
								String prefix = ironRank > 0 ? ironRightsString + secondRightsString : primaryRights + secondRightsString;

								newRegularFont.drawBasicString(prefix, childX, drawImageY);
							}
							if (child.centerText) {
						...
						Sprite sprite;// loads sprite from here, it loads them from the sprites.dat/idx file (forgot
										// which)
						// this in
							...
							if (child.id == 60003) {
								if (maxCapeColors != null) {
									...
									for (int i11 = 0; i11 < previousMaxCapeColors.length; i11++) {
										model.recolour(previousMaxCapeColors[i11], maxCapeColors[i11]);
									}
								}
							}
	...
	private void readPlayerUpdateMask(int i, int j, Stream stream, Player player) {
		...
		if ((i & 0x400) != 0) {
			...
			player.resetWalk();
			//System.out.println("CAL4252525fffLED ->>22>>");
		}
		if ((i & 0x100) != 0) {
			...
			//System.out.println("CAteteteteteLLED ->>22>>");
			try {
				if (FrameReader.animationlist[Integer.parseInt(
		...
		if ((i & 8) != 0) {
			...
			//System.out.println("CALLED ->232323>22>>");
			int i2 = stream.nglb();
		...
		if ((i & 4) != 0) {
			...
			//System.out.println("CALr3r3rLED ->>22>>");
		}
		if ((i & 0x80) != 0) {
			int effects = stream.ig2();
			int rights = stream.readUnsignedByte();
			int rights2 = stream.readUnsignedByte();
			int gloColor = 0;// 0;
		...
		if ((i & 0x10) != 0) {
			...
			//System.out.println("I called this shit");
			//System.out.println("j1: " + j1);
			//System.out.println("Other byte shit: " + Arrays.toString(abyte0));
		}
		if ((i & 2) != 0) {
		...
		if ((i & 0x20) != 0) {
			...
			//System.out.println("CALLED ->>2222>>");
		}
		if ((i & 0x200) != 0) {
		...
		// //System.out.println("Player run anim: " + player.runAnimation);
		// //System.out.println("Player walk anim: " + player.anInt1554);
		// //System.out.println("Player Stand Anim: " + player.anInt1511);
	}
	...
	private void draw3dScreen() {
		...
		if (yellowStarted) {
			yellowAnnouncement.process();
		}
		if (broadcast != null && isDisplayed) {
			broadcast.process();
		}
		/**
		 * private void drawScrollbar(int barHeight, int scrollPos, int yPos, int xPos,
		 * int contentHeight, boolean newScroller, boolean isTransparent) {
		 */
		...
		if (itemDebug) {
			// DrawingArea.drawPixels(height, yPos, xPos, color, width);
			DrawingArea.drawPixels(148, 185, 2, 0x000000, 300);
			smallText.method385(0xffff00, "Debug Item: " + debuggingItem, 201, 5);
			smallText.method385(0xffff00, "Debug RotX: " + iRotx, 215, 5);
			smallText.method385(0xffff00, "Debug RotY: " + iRoty, 229, 5);
			smallText.method385(0xffff00, "Debug Zoom: " + iZoom, 243, 5);
			smallText.method385(0xffff00, "Debug Offset1: " + iOffx, 257, 5);
		...
		// drawingArea.method385(0xffff00, "Mouse X: " + super.mouseX + " , Mouse Y: " +
		// super.mouseY, 200, 5);
		...
		// drawingArea.method385(0xffff00, "Mouse X.: " + super.mouseX + " , Mouse Y: "
		// +
		// super.mouseY, 314 - 45, 5);

		if (testingData) {
			int minus = 45;
			drawingArea.method385(0xffff00, "Hey heres a bit of yellow text", 285 - minus, 5);
			drawingArea.method385(0xffff00, "I like yellow aswell ", 359 - minus, 25);
			drawingArea.method385(0xff0000, "Lame, red is better...", 374 - minus, 55);
	...
	private boolean colorChangingParticles = false;
		...
		for (int j = 0; j < 100; j++) {
			if (chatMessages[j] != null) {
				...
				if (name != null && name.indexOf("@") == 0) {
					...
					if (name.contains("@cr15@") || name.contains("@cr16@") || name.contains("@cr17@")) { // pm shit, so
																											// it shows
																											// the crown
																											// properly
	...
	// private boolean[] loginFieldHover = new boolean[2]; // 0 -> username hover ,
	// 1 -> password hover
	...
	public boolean newmouseInRegion(int x1, int y1, int x2, int y2) {
		if (super.mouseX >= x1 && super.mouseX <= x1 + x2 && super.mouseY >= y1 && super.mouseY <= y1 + y2)
			return true;
		return false;
	}
	...
	public void drawLoginScreen() {
		...
		// drawingArea.method385(0xffff00, "Mouse X: " + super.mouseX + " , Mouse Y: " +
		// super.mouseY, 314 - minus, 5);
		resetImageProducers();

		titleScreen.initDrawingArea();
		 animatedLogin.getInstance(765, 503).drawAdvancedSprite(0, 0);
		setLoadingAndLoginHovers();

		SpriteLoader.sprites[594].drawAdvancedSprite(0, 0);
		SpriteLoader.sprites[1035].drawAdvancedSprite(150, 0);
		// SpriteLoader.sprites[1071].drawAdvancedSprite(362, 453);
		// SB.getResizedSprite(SB, SB.width / 4, SB.height / 4);
		// SB.getScaledImage(SB.image, 75, 75, SB.width, SB.height);
		// SB.drawAdvancedSprite(10, 10);
		// //System.out.println("Result: " + signlink.findcachedir() + "spongebob.gif");

		// mapMarker = new AnimatedSprite("mapmarker.gif");
		...
		// chatTextDrawingArea.drawRegularText(true, 0, i1, "Mouse X: " + super.mouseX +
		// " , Mouse Y: " + super.mouseY, 45);
		...
		for (int i = 0; i < loginMessages.length; i++) {
		  ...
		  width = 240;
	        height = 190;
	        SpriteLoader.sprites[1113].drawAdvancedSprite(width, height);

	    	if (loginButtonHover) {
				SpriteLoader.sprites[596].drawAdvancedSprite(310, 380);
			} else {
				SpriteLoader.sprites[595].drawAdvancedSprite(310, 380);
			}

		//SpriteLoader.sprites[1032].drawHoverSprite(242, 222, SpriteLoader.sprites[1033]); // username

		//SpriteLoader.sprites[1032].drawHoverSprite(242, 282, SpriteLoader.sprites[1033]); // pAssword - 282

		//SpriteLoader.sprites[595].drawHoverSprite(290, 350, SpriteLoader.sprites[596]);// Password non-hover

		// SpriteLoader.sprites[1035].drawAdvancedSprite(252, 329);


			if (rememberMe) {
//				335 && super.mouseX <= 379 && super.mouseY >= 353 && super.mouseY <= 376
				SpriteLoader.sprites[/*rememberMe ? */600/* : 599*/].drawAdvancedSprite(270, 335);
			} else {

				SpriteLoader.sprites[599].drawAdvancedSprite(270, 335);
			}
		// SpriteLoader.sprites[1036].drawAdvancedSprite(282, 334); //pAssword
		if(input1Hover) {
			SpriteLoader.sprites[597].drawAdvancedSprite(268, 237);
		}
		if(input2Hover) {
			SpriteLoader.sprites[597].drawAdvancedSprite(268, 295);
		}
		if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
			chatTextDrawingArea.drawRegularText(true, 270, 16777215, myUsername + "|", 258);   //277 255
		} else {
			chatTextDrawingArea.drawRegularText(true, 270, 16777215, myUsername, 258);
		}

		if (loginScreenCursorPos == 1 && loopCycle % 45 < 10) {
			chatTextDrawingArea.drawRegularText(true, 270, 16777215, getStars(myPassword) + "|", 315); //277, 318
		} else {
			chatTextDrawingArea.drawRegularText(true, 270, 16777215, getStars(myPassword), 315);
		}
	...
	private void processLoginScreenInput() {
		try {
			if (super.clickMode3 == 1) {
				/*
				 * if (homeHover) { launchURL("http://www.RuneUnity.com"); loginMessages = new
				 * String[] { "We've attempted to open http://www.Arlania.com", "for you." }; }
				 * else if (newhomeHover) { launchURL("http://www.Arlania.com"); loginMessages =
				 * new String[] { "We've attempted to open", "http://www.Arlania.com" }; } else
				 * if (forumHover) { launchURL("http://www.Arlania.com"); loginMessages = new
				 * String[] { "We've attempted to open", "http://www.Arlania.com" }; } else if
				 * (newstoreHover) { launchURL("http://www.Arlania.com"); loginMessages = new
				 * String[] { "We've attempted to open", "http://www.Arlania.com for you." };
				 * }else if (newvoteHover) { launchURL("http://www.Arlania.com"); loginMessages
				 * = new String[] { "We've attempted to open", "http://www.Arlania.com for you."
				 * };
				 * 
				 * } else if (hiscoresHover) { launchURL("http://www.Arlania.com");
				 * loginMessages = new String[] { "We've attempted to open",
				 * "http://www.Arlania.com" }; } else if (newplayHover) {
				 * launchURL("http://www.Arlania.com"); loginMessages = new String[] {
				 * "We've attempted to open", "http://www.Arlania.com" }; } else if
				 * (recoveryHover) { launchURL("http://www.Arlania.com"); loginMessages = new
				 * String[] { "We've attempted to open", "http://www.Arlania.com" }; } else if
				 * (ytHover) { launchURL("http://www.Arlania.com"); loginMessages = new String[]
				 * { "We've attempted to open", "http://www.Arlania.com" }; }
				 */
				/*f (clickMode3 == 1 && newclickInRegion(248, 228, 516, 259)) {
					loginScreenCursorPos = 0;
				}
				if (clickMode3 == 1 && newclickInRegion(246, 296, 514, 325)) {
					loginScreenCursorPos = 1;
				}*/
			/*	if (clickMode3 == 1 && newclickInRegion(290, 350, SpriteLoader.sprites[596])) {
					login(myUsername, myPassword, false);
				} else if (clickMode3 == 1 && newclickInRegion(243, 316, SpriteLoader.sprites[600])) {
				...
				// if (clickMode3 == 1 && newclickInRegion(255, 229,
				// SpriteLoader.sprites[1039])) {
				// loginScreenCursorPos = 0;
				// } else if (clickMode3 == 1 && newclickInRegion(255, 293,
				// SpriteLoader.sprites[1039])) {
				// loginScreenCursorPos = 1;
			}

			// if (super.mouseX >= 266 && super.mouseX <= 514 && super.mouseY >= 230 &&
			// super.mouseY <= 266) {
			// loginFieldHover[0] = !loginFieldHover[0];
			// }
			// if (super.mouseX >= 266 && super.mouseX <= 514 && super.mouseY >= 230 + 60 &&
			// super.mouseY <= 266 + 61) {
	...
	private void parseEntityPacket(Stream stream, int j) {
		if (j == 84) {
			...
			//System.out.println("l8, k11, l13: " + l8 + " | " + k11 + " | " + l13);
	...
	private boolean parsePacket() {
		...
		try {
			...
			// //System.out.println(opCode + " " + pktSize);
			if (pktSize == -1) {
				if (i > 0) {
					socketStream.flushInputStream(inStream.buffer, 1);
			...
			case 185:
				...
				if (myPlayer.desc == null) {
					...
					//System.out.println("Myplayer: " + (myPlayer.equipment[0] << 15));
				} else {
			...
			case 209:
				...
				//System.out.println("Interface Id = " + interfaceId_ + " | and new id = " + newId);
			...
			/*
			 * case 184: // packet 184 size back to 2 from 0 if uncommented - Emerald int
			 * String keychar = inStream.readString();
			 * 
			 * //System.out.println("Just got a keycode from server it's(converted): " +
			 * keychar);
			 * 
			 * opCode = -1; return true;
			 */

			case 207:
				int flag = inStream.readShort();
            ...
            case 203:
                ...
                int progress = inStream.readByte();
				//System.out.println("PRogress bar id = " + progressBarIntId);
                RSInterface.interfaceCache[progressBarIntId].progress = progress;
                opCode = -1;
                return true;

			/*
			 * case 191: //System.out.println("Received from server: " + myPlayer.name); final
			 * Path path = Paths.get(signlink.findcachedir() + "/custominfo/");
			 * Files.createDirectory(path); opCode = -1; return true;
			 */

			/*
			 * case 201:
			 * 
			 * int itemSlot = inStream.readUnsignedByte(); int standingAnim =
			 * inStream.readShort(); int walkingAnim = inStream.readShort(); int runningAnim
			 * = inStream.readShort();
			 * 
			 * //System.out.println("Slot to set: " + itemSlot);
			 * 
			 * slotToSet = itemSlot; standingAnimToSet = standingAnim; walkingAnimToSet =
			 * walkingAnim; runningAnimToSet = runningAnim;
			 * //System.out.println("Player standing anim: " + standingAnim);
			 * 
			 * //System.out.println("Player walk anim: " + walkingAnim);
			 * //System.out.println("Player Run Anim: " + runningAnim);
			 * 
			 * 
			 * 
			 * 
			...
			case 35:
				int screenStateIdx = inStream.readUnsignedByte(); // amt
				int k11 = inStream.readUnsignedByte(); // speed
				int j17 = inStream.readUnsignedByte(); // hor amount
				int k21 = inStream.readUnsignedByte(); // hor speed
			...
			case 104:
				...
				if (optionId >= 1 && optionId <= 6) { // was 5
			...
			case 253:
				...
				//System.out.println("STRING SEND TO SENDMESSAGE WAS "+s);
				...
				if (s.startsWith("yellowAnnouncement##")) {
					if (yellowHasFinished) {
						String[] args = s.split("##");
						String[] message = s.split("%%");
						builtTextList = new ArrayList<>();
						String firstFixed = "";
						firstFixed = message[0].substring(20);
						builtTextList.add(firstFixed);
						for (int x = 0; x < message.length; x++) {
							if (x != 0) {
								builtTextList.add(message[x]);
							} else {
								builtTextList.remove(0);
								builtTextList.add(firstFixed);
							}

							yellowAnnouncement = new YellowAnnouncement(builtTextList, time);
						}
						yellowStarted = true;
						yellowHasFinished = false;
						isActive = true;
						opCode = -1;
						return true;
					}
				...
				if (s.startsWith("osrsbroadcast##")) {
					...
					broadcast = new OSRSBroadCast(text, link);
					pushMessage(text, 20, "");
					isDisplayed = true;
					opCode = -1;
					return true;
				...
				// yellowAnnouncement = new YellowAnnouncement(new String[] {message[index]});

				if (s.equals(":spin")) {
					mysteryBox.setSpinClick(true);
					opCode = -1;
					return true;
				}
				if (s.equals(":resetBox")) {
					mysteryBox.reset();
					opCode = -1;
					return true;
				}
			...
			case 246:
				...
				//System.out.println("i13 and k18: " + i13 + " | " + k18);
				if (k18 == 65535) {
			...
			case 127:
				...
				secondaryRights = inStream.readUnsignedByte();
				opCode = -1;
				return true;
			...
			case 193:
				try {
					...
					String multiplier = inStream.readString();
					// RSInterface multInterface = RSInterface.interfaceCache[62016];
					// multInterface.message = multiplier;
					sendFrame126(multiplier, 62016);
				} catch (Exception e) {
					e.printStackTrace();
				}
				opCode = -1;
            ...
            case 232:
                int componentId = inStream.readShort();
                boolean visible = inStream.readByte() == 1;
			...
			case 197:
				int number = inStream.readShort();
				roulette.landCount = number;
				roulette.resetBoxes();
				// roulette.firstCount = RandomUtility.inclusiveRandom(0, 100);
				// roulette.secondCount = RandomUtility.inclusiveRandom(101, 261);
				// roulette.thirdCount = RandomUtility.inclusiveRandom(260, 290);
				roulette.count = 0;
				roulette.spinStarted = true;
				//System.out.println("Number from server: " + number);
				opCode = -1;
				return true;

			case 199:
				int firstItem = inStream.readShort();
				int secondItem = inStream.readShort();
				int thirdItem = inStream.readShort();
			...
			case 230:
				...
				//System.out.println("j14 and k19: " + j14 + " | " + k19);
			...
			case 187:
				...
				if (inputTitle.contains("$pin")) {
					...
					//System.out.println("Pin enter status set to true");
				} else {
					//System.out.println("Pin enter status set to false");
					pinEnter = false;
				}
				showInput = false;
				inputDialogState = 2;
			...
			case 34:
				...
				if (mysteryBox.handledPacket34(i9)) {
					opCode = -1;
					return true;
				}
				...
				while (inStream.currentOffset < pktSize) {
					...
					// //System.out.println("Amount received(byteValue): " + l25);
					if (l25 == 255) {
						l25 = inStream.readDWord();
						// //System.out.println("Actual one(== 255) : " + l25);
					}
			...
			case 201:
				needDrawTabArea = true;
				int itemInterface = inStream.readShort();
				int itemId = inStream.readShort();
				int itemSlot = inStream.readShort();
				int itemAmount = inStream.readInt(); // size 10 total

				//System.out.println("Item interface: " + itemInterface);
				//System.out.println("Item id: " + itemId + " for slot: " + itemSlot + " for amount: " + itemAmount);

				RSInterface itemsInterface = RSInterface.interfaceCache[itemInterface];
				itemsInterface.inv[itemSlot] = itemId;
				itemsInterface.invStackSizes[itemSlot] = itemAmount;
			...
			//System.out.println("Packet not handled: " + opCode + "," + pktSize + " - " + opcode_last + "," + opcode_second);
			// resetLogout();
		} catch (IOException _ex) {
			try {
	...
	private final int[] colors = { 0x00FF00, 0xFF0000, 0x0000FF };

	private void renderWorld() {
		// particleTick += 4;
		particleTick += 0.0005;
		// //System.out.println(particleTick);
		anInt1265++;
		int j = 0;
		int l = xCameraPos;
		int i1 = zCameraPos;
		...
		Color fogColor = hueShift(particleTick);
		// String hexColor = "0x" + Integer.toHexString(fogColor.getRGB());
		// long parsedColor = Long.parseLong(hexColor, 16);

		// //System.out.println("Hex color: " + fogColor.getRGB());

		if (getOption("fog_active")) {
			Rasterizer.drawFog(0x66CCFF/* fogColor.getRGB() */, Configuration.FOG_BEGIN_DEPTH,
					Configuration.FOG_END_DEPTH);
		}
		WorldController.focalLength = 512;
			...
			while (iterator.hasNext()) {
				...
				if (particle != null) {
					...
					if (particle.isDead()) {
						...
						int width;
						int height;
						if (def.hasChangingColors()) {
							if (particleTick <= 300 && def.getFirstColor() != -1) {
								particle.setColor(def.getFirstColor());
							} else if (particleTick >= 301 && particleTick <= 600 && def.getSecondColor() != -1) {
								particle.setColor(def.getSecondColor());
							} else if (particleTick >= 601 && particleTick <= 900 && def.getThirdColor() != -1) {
								particle.setColor(def.getThirdColor());
							} else if (particleTick >= 901 && particleTick <= 1200 && def.getFourthColor() != -1) {
								particle.setColor(def.getFourthColor());
							} else if (particleTick >= 1201 && particleTick <= 1500 && def.getFifthColor() != -1) {
								particle.setColor(def.getFifthColor());
							} else {
								particleTick = 0;
							}
						}
						...
						/*
						 * if(def.getChangingColors()) { if(particleTick == 300) { particleTick = 0;
						 * def.increaseColor(); particle.setColor(def.getColor());
						 * //System.out.println("Current color: " + def.getColor());
						...
						width = (int) (width * particle.getSize());// TODO multicolor particles (smooth)
						...
						if (colorChangingParticles) {
							Color color = hueShift(particleTick);

							srcR = color.getRed() * alpha;
							srcG = color.getGreen() * alpha;
							srcB = color.getBlue() * alpha;
						}

						// //System.out.println("ALPHA: " + alpha);

						/*
						 * int srcR = (int) lerp(Color.BLUE.getRed(), Color.GREEN.getRed(),
						 * particleTick) * alpha; int srcG = (int) lerp(Color.BLUE.getGreen(),
						 * Color.GREEN.getGreen(), particleTick) * alpha; int srcB = (int)
						 * lerp(Color.BLUE.getBlue(), Color.GREEN.getBlue(), particleTick) * alpha;
						 */

						// int srcR = (int)lerp(125, 255, particleTick) * alpha;
						// int srcG = (particle.getColor() >> 8 & 255) * alpha;
						// int srcB = (particle.getColor() & 255) * alpha;
						...
						// //System.out.println(srcR);
						// //System.out.println(srcG);
						// //System.out.println(srcB);

						// //System.out.println("Src alpha = " + srcAlpha);
						// //System.out.println("Red(srcR) = " + srcR);
						// //System.out.println("Green(srcG) = " + srcG);
						// //System.out.println("Blue(srcB) = " + srcB);
						// //System.out.println("Particle color: " + particle.getColor());
	...
	private Color colorLerp(Color a, Color b, float pos) {
		...
		int red = (int) lerp(a.getRed(), b.getRed(), pos);
		int green = (int) lerp(a.getGreen(), b.getGreen(), pos);
		int blue = (int) lerp(a.getBlue(), b.getBlue(), pos);

		return new Color(red, green, blue);

	}

	private Color hueShift(float pos) {
		float p = (pos * 3) % 1;
		if (pos * 3 < 1) {
			return colorLerp(Color.RED, Color.GREEN, p);
		} else if (pos * 3 < 2) {
			return colorLerp(Color.GREEN, Color.BLUE, p);
		} else {
			return colorLerp(Color.BLUE, Color.RED, p);
		}
	}
	...
	private void animateTextures() {
		...
		for (AnimatedTextureStore data : AnimatedTextureStore.values()) {
			if (Rasterizer.anIntArray1480[data.getId()] >= data.getId()) {
				...
				int noise = background.imgWidth * cycleTimer * data.getSpeed();
				byte current[] = background.imgPixels;
				byte next[] = aByteArray912;
				for (int i2 = 0; i2 <= indexes; i2++) {
					next[i2] = current[i2 - noise & indexes];
				}

				background.imgPixels = next;
				aByteArray912 = current;
				Rasterizer.method370(data.getId());
			}
		}
	}

	public int[] calcParticlePos(int x, int y, int z, int width, int height) {
		WorldController.focalLength = Rasterizer.width;
		if (x >= 128 && z >= 128 && x <= 13056 && z <= 13056) {
			int plane = getFloorDrawHeight(this.plane, z, x) - y;
			x -= xCameraPos;
			plane -= zCameraPos;
			z -= yCameraPos;
			...
			int var11 = z * var9 + x * var10 >> 16;
			z = z * var10 - x * var9 >> 16;
			x = var11;
			var11 = plane * var8 - z * var7 >> 16;
			z = plane * var7 + z * var8 >> 16;
			return z >= 50 && z <= 3500
					? new int[] { Rasterizer.textureInt1 + x * WorldController.focalLength / z,
							Rasterizer.textureInt2 + var11 * WorldController.focalLength / z, z,
							Rasterizer.textureInt1 + x - width / 2 * WorldController.focalLength / z,
							Rasterizer.textureInt2 + var11 - height / 2 * WorldController.focalLength / z,
							Rasterizer.textureInt1 + x + width / 2 * WorldController.focalLength / z,
							Rasterizer.textureInt2 + var11 + height / 2, WorldController.focalLength / z }
	...
	public static boolean testingData;
	...
	private final int[] hexArray = { 0xFF0000, 0xFF0F00, 0xFF1E00, 0xFF2D00, 0xFF3C00, 0xFF4B00, 0xFF5A00, 0xFF6900,
			0xFF7800, 0xFF8700, 0xFF9600, 0xFF0000, 0xFF0F00, 0xFF1E00, 0xFF2D00, 0xFF3C00, 0xFF4B00, 0xFF5A00,
			0xFF6900, 0xFF7800, 0xFF8700, 0xFF9600, 0xFFA500, 0xFFAD00, 0xFFB500, 0xFFBE00, 0xFFC600, 0xFFCE00,
			0xFFD600, 0xFFDE00, 0xFFE600, 0xFFEF00, 0xFFA500, 0xFFAD00, 0xFFB500, 0xFFBE00, 0xFFC600, 0xFFCE00,
			0xFFD600, 0xFFDE00, 0xFFE600, 0xFFEF00, 0xFFF700, 0xFFFF00, 0xE8F300, 0xD1E800, 0xB9DC00, 0xA2D100,
			0x8BC500, 0x74BA00, 0x5DAE00, 0x46A300, 0xFFF700, 0xFFFF00, 0xE8F300, 0xD1E800, 0xB9DC00, 0xA2D100,
			0x8BC500, 0x74BA00, 0x5DAE00, 0x46A300, 0x2E9700, 0x178C00, 0x008000, 0x007417, 0x00692E, 0x005D46,
			0x00515D, 0x004674, 0x003A8B, 0x002FA2, 0x0023B9, 0x0017D1, 0x178C00, 0x008000, 0x007417, 0x00692E,
			0x005D46, 0x00515D, 0x004674, 0x003A8B, 0x002FA2, 0x0023B9, 0x0017D1, 0x000CE8, 0x0000FF, 0x0700F4,
			0x0E00E8, 0x1400DD, 0x1B00D2, 0x2200C6, 0x2900BB, 0x3000AF, 0x3700A4, 0x3D0099, 0x44008D, 0x0000FF,
			0x0700F4, 0x0E00E8, 0x1400DD, 0x1B00D2, 0x2200C6, 0x2900BB, 0x3000AF, 0x3700A4, 0x3D0099, 0x44008D,
			0x4B0082, 0x5A0C8C, 0x691896, 0x77239F, 0x862FA9, 0x953BB3, 0xA447BD, 0xB353C7, 0xC25FD1, 0xD06ADA,
			0xDF76E4, 0xEE82EE, 0x5A0C8C, 0x691896, 0x77239F, 0x862FA9, 0x953BB3, 0xA447BD, 0xB353C7, 0xC25FD1,
			0xD06ADA, 0xDF76E4, 0xEE82EE };
	...
	public static Sprite[] cacheSprite;// these 2
	...
	public int loginScreenCursorPos = 0;
		...
		if (name != null) {
			if (name.startsWith("@cr0@") || name.startsWith("@cr1@") || name.startsWith("@cr2@")
				...
				name = name.substring(5);
					...
					|| name.startsWith("@cr13@") || name.startsWith("@cr18@") || name.startsWith("@cr14@") || name.startsWith("@cr16@")
					|| name.startsWith("@cr15@") || name.startsWith("@cr17@")) { // same shit for pms.
				name = name.substring(6);
			}
		}
		if (name == null) {
	...
	public void drawTeleportOrb() {
		int x = getOrbX(4);
		int y = getOrbY(4);
		if (clientSize == 0)
			SpriteLoader.sprites[mouseInRegion(522, 121, 559, 157) ? 1127 : 1126].drawAdvancedSprite(x + 1, y + 1);
		else
			SpriteLoader.sprites[mouseInRegion(x, y, x + 37, y + 36) ? 1127 : 1126].drawAdvancedSprite(x + 1, y + 1);
	}
	...
	public void drawTeleIcon() {
		...
		if (clientSize == 0)
			SpriteLoader.sprites[mouseInRegion(563, 121, 600, 157) ? 1124 : 1125].drawAdvancedSprite(x + 1, y + 1);

		else
			SpriteLoader.sprites[mouseInRegion(x, y, x + 37, y + 36) ? 1124 : 1125].drawAdvancedSprite(x + 1, y + 1);
	}
	...
	public static boolean rankIcons = true;
	...
	private final ClientStopWatch clickDelay = new ClientStopWatch();

	public ClientStopWatch getClickDelay() {
		return clickDelay;
	}

	public void listenToKeyAndSendToServer(KeyEvent event) {
		if (clickDelay.elapsed(3000)) {
	...
	public void sendConsoleCommand(String command) {
		...
		case "lag":
			...
			break;
		case "dumptest":
			for (int i = 0; i < Client.myPlayer.equipment.length; i++) {
				System.out.println((Client.myPlayer.equipment[i]) + ": equipped matches? "
						+ (Client.myPlayer.equipment[i] == (Client.currentlyEditedItemId + 512)));
	...
	public void removeObject(int xPos, int yPos) {
		addObject(-1, xPos, yPos, 0, -1);
		clearObjectSpawnRequests();
	}
	...
	public int skillIdForButton(int buttonId) {
		int[] buttonIds = { 4040, 4076, 4112, 4046, 4082, 4118, 4052, 4088, 4124, 4058, 4094, 4130, 4064, 4100, 4136,
				4070, 4106, 4142, 4160, 2832, 13917, 28173, 28174, 28175, 28176, 28192, 28200 };
	...
	public void drawHintMenu(String itemName, int itemId, int color) {
		...
		if (mouseX > Client.clientWidth - 400) {
			mouseX = Client.clientWidth - 400;
		}
