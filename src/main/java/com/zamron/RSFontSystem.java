...
public class RSFontSystem extends DrawingArea {
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
				...
				if (startIndex == -1) {
					...
					if (underlineColor != -1) {
						DrawingArea474.drawHorizontalLine(drawX, drawY + baseCharacterHeight, lineWidth,
								underlineColor);
					}
					drawX += lineWidth;
				}
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
				...
				if (startIndex == -1) {
					...
					if (strikethroughColor != -1) {
						DrawingArea474.drawHorizontalLine(drawX, drawY + (int) ((double) baseCharacterHeight * 0.7),
	...
	public static String handleOldSyntax(String text) {
		...
		text = replace(text, "@pr2@", "<col=7f1fe0>");
		...
		if (text != null) {
			...
			while (matcher.find()) {
				text = replace(text, text.substring(matcher.start(), matcher.end()),
						String.format("<col=%s>", text.substring(matcher.start() + 1, matcher.end() - 1)));
	...
	public void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_37_ > 0 && i_38_ > 0) {
			try {
				createCharacterPixels(DrawingArea.pixels, fontPixels[character], i_39_, i_43_, i_40_, i_37_, i_38_,
						i_41_, i_42_);
			} catch (Exception e) {
