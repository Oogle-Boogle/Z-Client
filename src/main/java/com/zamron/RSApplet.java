...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
	...
	public int getHeight() {
		return (Client.instance.loggedIn && Client.clientSize == 1 && Client.paneClosed > 0) ? super.getHeight() - 55
				: super.getHeight();
	}

	@Override
	public int getWidth() {
		return (Client.instance.loggedIn && Client.clientSize == 1 && Client.paneClosed > 0) ? super.getWidth() - 5
				: super.getWidth();
	}
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if(this.controlling && i == KeyEvent.VK_V) {
			Client.instance.paste();
		}
		
		if(this.controlling && i == KeyEvent.VK_C) {
			Client.instance.copy();
		}
			...
			switch (i) {
				case KeyEvent.VK_LEFT:
					...
					if(controlling)
						...
						Client.iRotx -= (Client.iRotx - 10 > 0 ? 10 : 0);
				break;
				case KeyEvent.VK_UP:
					if(shifting)
						Client.iZoom += (Client.iZoom + 100 < 2050 ? 100 : 0);
					else if(controlling)
						Client.iOffy += 1;
					else
						Client.iRoty -= (Client.iRoty - 10 > 0 ? 10 : 0);
				// Client.iRoty += (Client.iRoty + 10 < 2050 ? 10 : 0);
				break;
				case KeyEvent.VK_RIGHT:
					...
					if(controlling)
						...
						Client.iRotx += (Client.iRotx + 10 < 2050 ? 10 : 0);
				break;
				case KeyEvent.VK_DOWN:
					if(shifting)
						Client.iZoom -= (Client.iZoom - 100 > 0 ? 100 : 0);
					else if(controlling)
						Client.iOffy -= 1;
					else
						Client.iRoty += (Client.iRoty + 10 < 2050 ? 10 : 0);
